Interfaces.Party = function (scene, properties = {}) {
    this.scene = scene;
...
// Add interface
Interfaces.Party.prototype.append = function (type) {
    ...
    // sair da interface
    this.scene.partyBackButton = new Phaser.Button(this.scene, {
        x: 221,
        y: 198,
...
// Limpar interface
Interfaces.Party.prototype.clear = function (destroyBg) {
...
// Add base dos slots
Interfaces.Party.prototype.appendPartySlots = function () {
    this.scene.slots = [];
    ...
    this.scene.database.interface.party.slots.positions.forEach((position, index) => {
        ...
        this.scene.slots[index] = new Phaser.Button(this.scene, {
            x: position.x,
            y: position.y,
...
Interfaces.Party.prototype.appendSlotElements = function () {
    ...
    for (let i = 0; i < 6; i ++) {
        ...
        if (monsters[i].id > 0) {
            ...
            this.scene.slots[i].elements.icon.setInteractive({
                useHandCursor: true
            });
            ...
            this.scene.slots[i].elements.icon.setScrollFactor(0);
...
// Add tooltip
Interfaces.Party.prototype.appendPartyTooltip = function (index) {
    ...
    switch (this.type) {
        ...
        case "party": {
            this.scene.btn.status = new Phaser.Button(this.scene, {
                ...
                on: {
                    click: () => {
                        this.clear(true);
                        this.scene.interfacesHandler.status = new Interfaces.MonsterStatus(this.scene);
        ...
        case "item": {
            ...
            this.scene.btn.action = new Phaser.Button(this.scene, {
                x: this.scene.tooltip.x + this.scene.database.interface.party.btntooltips.positions.once.x,
                y: this.scene.tooltip.y + this.scene.database.interface.party.btntooltips.positions.once.y + add,
                spritesheet: "button_new",
                on: {
        ...
        case "battleParty": {
            ...
            this.scene.btn.trade = new Phaser.Button(this.scene, {
...
// Limpar tooltip
Interfaces.Party.prototype.clearPartyTooltip = function () {
...
Interfaces.Party.prototype.treatTooltipFrame = function (index) {
    switch (index) {
        case 0:
        case 1:
        {
...
Interfaces.Party.prototype.elementsFollowSlot = function (slot) {
...
Interfaces.Party.prototype.slotOnDragEnd = function (slot, index) {
        ...
        if (slotPosition.x >= this.scene.slots[i].sprite.x && slotPosition.x <= this.scene.slots[i].sprite.x + width && 
           slotPosition.y >= this.scene.slots[i].sprite.y && slotPosition.y <= this.scene.slots[i].sprite.y + height) {
            ...
            if (this.scene.slots[i].isEmpty) {
                ...
                let new_order = _.clone(this.scene.Data.CurrentMonsters);
