...
Game.Overworld.interactWithObject = function () {
...
// chamar ação do objeto
Game.Overworld.callObjectAction = function (name, callback) {
...
Game.Overworld.automatizeAction = function (object, actions, callback) {
...
Game.Overworld.execInjectedAction = function (data) {
    switch (data.action) {
        case "heal_dialog": {
...
Game.Overworld.automatedAction = {
    empty: function (next) {
        next();
    },
    call: function (next) {
 ...
 // variaveis dinâmicas
Game.Overworld.dynamicVariables = {};

// Percorrer todos os domadores do mapa 
Game.Overworld.checkPlayerPositionTamer = function (events) {
...
Game.Overworld.checkPlayerTamerRange = function (axis, equalAxis, viewDirection, walkDirection, element, events) {
    ...
    if (canWalk) {
        let 
            pos = (this.player._data.position[axis] - this.object_data[element.name]._data.position[axis]) - 1,
            script = [];
...
Game.Overworld.switchInteractionLock = function () {
